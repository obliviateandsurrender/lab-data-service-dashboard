...
def signup():
    if request.method == 'POST':
        ...
        if email and body and name:
            msg = Message('Access Request', sender=ADMINS[0], recipients=ADMINS)
            msg.body = """
            From: %s 
            Googe Email: %s;
            Reason: %s
...
def lds_connect1():
    ...
    os.environ['http_proxy'] = os.environ['https_proxy'] = ''
    r = requests.get(LDS+payload, verify = False)
<<<<<<< HEAD
    
=======
    print r.headers['X-Pagination']
>>>>>>> 45d4f1c4c0eb840ed7b2a223f27ed87a995b0422
    try:
        page = r.headers['X-Pagination']
    except:
        page = None
    os.environ['http_proxy'] = http_proxy
    os.environ['https_proxy'] = https_proxy
    
    return jsonify(success=True, data=r.json(), page=page), r.status_code
#+END_SRC
**** Test Cases
#+NAME:
#+BEGIN_SRC python
...
def lds_connect2():
    ...
    os.environ['http_proxy'] = os.environ['https_proxy'] = ''
    headers = {'content-type': 'application/json'}
    r = requests.post(LDS+call,data=data,
                          verify = False,
                          headers=headers)
    os.environ['http_proxy'] = http_proxy
    os.environ['https_proxy'] = https_proxy
    return jsonify(success=True, message="Life is broken"), r.status_code
...
def lds_connect3():
    ...
    try:
        ...
        call = json.loads(payload)['call']+'s?key=defaulykey'
    data = json.dumps(payment)    
    ...
    os.environ['http_proxy'] = os.environ['https_proxy'] = ''
    headers = {'content-type': 'application/json'}
    r = requests.put(LDS+call, data=data,
...
def lds_connect4():
    ...
    r = requests.delete(LDS+call+'?'+payment+'&'+key,
                            verify = False)
    os.environ['http_proxy'] = http_proxy
    os.environ['https_proxy'] = https_proxy
...
from runtime.rest.app import app, db, requires_guest_auth, \
     requires_admin_auth, disable_access_on_hash, ADMINS, mail, \
     LDS
from runtime.rest.app.users.db import User, Roles
import os
import json
